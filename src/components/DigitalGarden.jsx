...
import './DigitalGarden-v2.css';
import { renderMarkdownContent, renderMarkdownAsJSX } from './markdownRenderer.jsx';

// Sample notes untuk testing
const SAMPLE_NOTES = {
  'intro': {
    title: 'Pengenalan',
...
function DigitalGarden() {
  ...
  const renderSection = (sectionId) => {
    const notes = openNotes[sectionId];
    const content = sectionContents[sectionId];
    const markdownObjects = renderMarkdownContent(content);

    return (
      <div className="horizontal-section" key={sectionId}>
        <div className="section-scroll-area">
          {notes.length === 0 ? (
            <div className="section-empty-state">
              {renderMarkdownAsJSX(markdownObjects)}
            </div>
          ) : (
            <div className="notes-stack">
              {notes.map(noteId => {
                const note = SAMPLE_NOTES[noteId];
                if (!note) return null;
                return (
                  <div key={noteId} className="note-card">
                    <div className="note-card-header">
                      ...
                      <button
                        ...
                        onClick={() => handleCloseNote(sectionId, noteId)}
                      >
                        ✕
                      </button>
                    </div>
                      ...
                      <div className="note-card-links">
                        {note.links.map(linkId => (
                          <button
                            key={linkId}
                            className="note-link"
                            onClick={() => handleOpenNote(sectionId, linkId)}
                            title={SAMPLE_NOTES[linkId]?.title}
                          >
                            → {SAMPLE_NOTES[linkId]?.title.substring(0, 15)}
                          </button>
                        ))}
                      </div>
                    )}
  ...
  return (
    <div className="digital-garden-horizontal">
      <div className="horizontal-scroll-container" onScroll={handleScroll}>
        {[0, 1, 2, 3, 4].map(sectionId => renderSection(sectionId))}
